...
| `0005-dev-lua-encode-link.patch` | **NEW 2026-08-27, not upstream.** Companion to 0003: `do_encode/2` passes a cache link `{link, ID, #{...}}` through to `luerl:encode/2`, which raises `{badarg, Term}`, so a Lua device on `on/request` cannot price any request carrying links — which is nearly all of them. The `ensure_all_loaded` calls already in the map/list clauses do not cover it: they run on the converted `[{Key, Value}]` proplist, whose elements are pairs, not links. This is the FIRST of two causes that broke self-hosted bundling on stage and live (the node's own `~bundler@1.0/tx` POST 400s and the scheduler discards the result, so assignments vanish silently); the second is the write gate refusing the unsigned envelope, fixed in anyone-protocol `runtime/write-gate.lua`. Neither fix alone moves a byte. Numbered 0005 so `0004` keeps pointing at the dropped bundler-flag patch below and means one thing in this file. |
